...
def face_detect(face_image: str, is_local: bool) -> json:
    ...
    for face in detected_faces:
        res["id"] = face.face_id
        res["rectangle"] = json.loads(
            str(face.face_rectangle).replace("\'", "\""))
        res["emotion"] = json.loads(
...
def get_keywords(link):
    ...
    total_keywords = {k: v for k, v in sorted(
        total_keywords.items(), key=lambda item: item[1], reverse=True)}
...
def detect_change(history_all: dict, history_just: dict) -> bool:
    ...
    def get_full_history_keyword(history, count, min_val):
        ...
        top_all = {k: v for k, v in sorted(
            top_all.items(), key=lambda item: item[1], reverse=True)}
    ...
    MIN_VAL = 10
    keywords_all = get_full_history_keyword(history_all, 50, MIN_VAL)
    keywords_just = get_full_history_keyword(history_just, 10, MIN_VAL)
